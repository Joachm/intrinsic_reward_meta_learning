...
from my_gym.spaces import Space, Box, Discrete, MultiDiscrete, MultiBinary, Tuple, Dict

_Basemy_gymSpaces = (Box, Discrete, MultiDiscrete, MultiBinary)
__all__ = ["_Basemy_gymSpaces", "batch_space"]
...
def batch_space(space, n=1):
    ...
    Parameters
    ----------
    space : `my_gym.spaces.Space` instance
    ...
    Returns
    -------
    batched_space : `my_gym.spaces.Space` instance
        Space (e.g. the observation space) for a batch of environments in the
        vectorized environment.

    Example
    -------
    >>> from my_gym.spaces import Box, Dict
    >>> space = Dict({
    ...
    """
    if isinstance(space, _Basemy_gymSpaces):
        ...
        raise ValueError(
            ...
            "be a valid `my_gym.Space` instance.".format(type(space))
        )
