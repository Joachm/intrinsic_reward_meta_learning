...
from my_gym import error, logger
...
# Whitelist of plugins which can hook into the `my_gym.envs.internal` entry point.
plugin_internal_whitelist = {"ale_py.my_gym"}
    ...
    def make(self, **kwargs):
        ...
        if env.spec.max_episode_steps is not None:
            ...
            if self.order_enforce:
                from my_gym.wrappers.order_enforcing import OrderEnforcing
    ...
    def spec(self, path):
        if ":" in path:
            ...
            try:
                ...
                raise error.Error(
                    "A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `my_gym.make()`".format(
                        mod_name
                    )
                )
        else:
        ...
        try:
            ...
            if matching_envs:
                ...
                raise error.UnregisteredEnv(
                    "Algorithmic environment {} has been moved out of my_gym. Install it via `pip install my_gym-algorithmic` and add `import my_gym_algorithmic` before using it.".format(
                        id
                    )
                )
            elif env_name in toytext_envs:
                raise error.UnregisteredEnv(
                    "Toytext environment {} has been moved out of my_gym. Install it via `pip install my_gym-legacy-toytext` and add `import my_gym_toytext` before using it.".format(
                        id
                    )
                )
            else:
...
def load_env_plugins(entry_point="my_gym.envs"):
    ...
    for plugin in metadata.entry_points().get(entry_point, []):
        ...
        try:
            ...
            if attr is None:
                raise error.Error(
                    f"my_gym environment plugin `{module}` must specify a function to execute, not a root module"
                )
