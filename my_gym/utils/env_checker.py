...
def _check_nan(env: my_gym.Env, check_inf: bool = True) -> None:
...
def _check_returned_values(
    env: my_gym.Env, observation_space: spaces.Space, action_space: spaces.Space
) -> None:
    """
    Check the returned values by the env when calling `.reset()` or `.step()` methods.
    """
    # because env inherits from my_gym.Env, we assume that `reset()` and `step()` methods exists
    obs = env.reset()
    ...
    if isinstance(env, my_gym.GoalEnv):
...
def _check_spaces(env: my_gym.Env) -> None:
    """
    Check that the observation and action spaces are defined
    and inherit from my_gym.spaces.Space.
    """
    # Helper to link to the code, because my_gym has no proper documentation
    my_gym_spaces = " cf https://github.com/openai/my_gym/blob/master/my_gym/spaces/"

    assert hasattr(env, "observation_space"), (
        "You must specify an observation space (cf my_gym.spaces)" + my_gym_spaces
    )
    assert hasattr(env, "action_space"), (
        "You must specify an action space (cf my_gym.spaces)" + my_gym_spaces
    )

    assert isinstance(env.observation_space, spaces.Space), (
        "The observation space must inherit from my_gym.spaces" + my_gym_spaces
    )
    assert isinstance(env.action_space, spaces.Space), (
        "The action space must inherit from my_gym.spaces" + my_gym_spaces
    )


# Check render cannot be covered by CI
def _check_render(
    env: my_gym.Env, warn: bool = True, headless: bool = False
...
def check_env(env: my_gym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    """
    Check that an environment follows my_gym API.
    This is particularly useful when using a custom environment.
    Please take a look at https://github.com/openai/my_gym/blob/master/my_gym/core.py
    for more information about the API.
    It also optionally check that the environment is compatible with Stable-Baselines.
    :param env: The my_gym environment that will be checked
    :param warn: Whether to output additional warnings
    ...
    assert isinstance(
        env, my_gym.Env
    ), "Your environment must inherit from the my_gym.Env class cf https://github.com/openai/my_gym/blob/master/my_gym/core.py"
